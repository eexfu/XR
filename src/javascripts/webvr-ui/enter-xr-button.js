...
export default class EnterXRButton extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (!this.renderer) {
      console.error('EnterXRButton requires a THREE.WebGLRenderer');
    }
            ...
            return navigator.xr.requestSession('immersive-vr', XR_SESSION_OPTIONS)
              .then((session) => {
                ...
                session.addEventListener('end', () => {
                  this.xrSession = null;
                  this.__onStateChange(State.READY_TO_PRESENT);
                });
                
                // 配置渲染器使用XR会话
                if (this.renderer) {
                  this.renderer.xr.enabled = true;
                  return this.renderer.xr.setSession(session).then(resolve);
                }
    ...
    return new Promise((resolve, reject) => {
      if (this.options.onRequestStateChange(State.READY_TO_PRESENT)) {
        return this.options.beforeExit()
          .then(() => {
            if (this.xrSession) {
              return this.xrSession.end().then(() => {
                this.xrSession = null;
                resolve();
              });
            }
